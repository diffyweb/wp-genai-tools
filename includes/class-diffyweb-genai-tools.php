...
final class DiffyWeb_GenAI_Tools {
    ...
    public function render_pngquant_enabled_field() {
        $enabled = get_option( 'diffyweb_genai_tools_pngquant_enabled' );
        ?>
        <input type="checkbox" name="diffyweb_genai_tools_pngquant_enabled" value="1" <?php checked( 1, $enabled ); ?>>
        <p class="description">
            <?php esc_html_e( 'If checked, the plugin will attempt to optimize generated PNG images using the pngquant utility before uploading. This can significantly reduce file size.', 'diffyweb-genai-tools' ); ?>
            <br>
            <strong><?php esc_html_e( 'Requires:', 'diffyweb-genai-tools' ); ?></strong>
            <code>pngquant</code> <?php esc_html_e( 'must be installed on your server and the', 'diffyweb-genai-tools' ); ?>
            <code>exec()</code> <?php esc_html_e( 'PHP function must be enabled.', 'diffyweb-genai-tools' ); ?>
        </p>
        <?php
    }
    ...
    private function optimize_image_with_pngquant( $file_path ) {
        // 1. Check if exec is available on the server.
        if ( ! function_exists( 'exec' ) || false !== strpos( ini_get( 'disable_functions' ), 'exec' ) ) {
            error_log( 'Diffyweb GenAI Tools: exec() is disabled on this server. Cannot run pngquant.' );
            return false;
        }

        // 2. Check if pngquant is installed and in the system's PATH.
        // The @ suppresses errors if the command doesn't exist.
        // `2>&1` redirects stderr to stdout to capture potential errors.
        $version_output = @exec( 'pngquant --version 2>&1', $output_lines, $exit_code );
        if ( $exit_code !== 0 || empty( $version_output ) ) {
            error_log( 'Diffyweb GenAI Tools: pngquant command not found or not executable. Please ensure it is installed and in the system PATH.' );
            return false;
        }
        ...
        // --output: specify output file (same as input)
        // --quality: set a quality range
        // --skip-if-larger: don't save if the result is larger than the original
        // --: stop parsing options, treat next argument as a file
        $command = "pngquant --force --quality=65-80 --skip-if-larger --output {$file_path_escaped} -- {$file_path_escaped}";

        // Reset vars for the main execution
        $output_lines = [];
        $exit_code = 0;
        exec( $command . ' 2>&1', $output_lines, $exit_code );

        // 4. Check for success. pngquant exit codes: 0=success, 98=quality too low, 99=file larger.
        if ( ! in_array( $exit_code, [0, 98, 99] ) ) {
    ...
    private function upload_and_set_featured_image( $base64_image_data, $post_id, $post_title ) {
        ...
        // --- NEW: PNGQuant Optimization ---
        $pngquant_enabled = get_option( 'diffyweb_genai_tools_pngquant_enabled' );
        if ( $pngquant_enabled ) {
            $this->optimize_image_with_pngquant( $upload['file'] );
        }
        // --- END NEW ---
